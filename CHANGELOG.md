...
- Add `gpt-image-2.5-sunburst` (`GPT Image 2.5 Sunburst`) and `gpt-image-2.5-flare` (`GPT Image 2.5 Flare`) to `create image`, with 1080p / 1440p / 2160p generation, quality-specific aspect ratios, `low` / `medium` / `high` / `xhigh` / `max` detail levels, and up to 16 reference images; `gpt-image-2.5-flare` is now the default image model
...
- Add `minimax-h3` video model (`MiniMax H3`) — available in `create video`, `create reference`, and two-frame `create transition`, with 1440p, 5–15s durations, and up to 9 image / 3 video / 3 audio references
